...
### LLM (Larfe Language Model):

Language model is to predict the next token the next words based on the words you have typed so far or you have so far in your document, and why we call it as large because it has huge parameters (billions of parameters) and they are trained tons of data. The way LLM answer your questions are like you are talking with human (the way it's answer).
...
LLM will generate text and they use search to augmented the generation of the text. So in RAG, the user will asked some question then RAG will search on KD (knowledge database) about the context and send the context and the question to the LLM. Then the LLM will give you the answer for your questions based on the context in database.
